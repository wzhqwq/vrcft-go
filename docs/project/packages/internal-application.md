...
The coordinator serializes control and frame-path state. Monotonic plugin session identities make a coalesced running-to-running restart remove the prior tracking source. Avatar, merged-frame, and status subscriptions are bounded latest-value streams. Start begins plugins, then the ready coordinator, then OSC; a startup failure cancels and joins already-started work. Close stops OSC, cancels and joins the coordinator after clearing its runtime, then closes plugins; repeated close calls share the stable result. Lifecycle transitions snapshot OSC status after component calls without holding the Application lock.
## Error handling
