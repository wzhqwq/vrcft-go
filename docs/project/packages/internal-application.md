...
`Config.Logger` optionally accepts the root's slog logger. The coordinator forwards plugin log and state-change control events, preserving log level and reporting dropped plugin records. The production handler uses a bounded disk queue; it does not perform file I/O on the coordinator's frame path. A nil logger preserves existing embedding behavior.
