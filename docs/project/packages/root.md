...
The Wails-visible surface is `RuntimeAPI.GetStatus/GetDiagnostics/ReportFrontendError`, `PluginsAPI.List/GetConfig/SetEnabled/UpdateConfig`, and `SettingsAPI.Get/Validate/Save`, with versioned `vrcft:v1:runtime-status`, `vrcft:v1:plugins-changed`, and `vrcft:v1:settings-changed` events. The independently queried diagnostic snapshot exposes bounded redacted operational records and retains the startup failure with its stage, time and correlation ID. Frontend reporting accepts four allowlisted stages and at most ten records per minute. `main`, `App`, lifecycle callbacks, API attach/detach/consumer methods, stores, backend seams, and converters remain unbound implementation details.
## Owned data
...
Root owns a slog handler with 200 recent records and a bounded asynchronous disk queue. Startup opens `%APPDATA%/vrcft-go/logs/application.jsonl`; rotation retains five files total at 5 MiB per file. Disk failures preserve memory diagnostics and report the failure; queue overflow reports omitted disk records. Shutdown joins disk output after consumers stop. Frame payloads are never logged.
## Security boundaries
The closed Bind literal contains exactly the three API fields and no root App. DTO conversion allowlists fields, deep-copies slices/JSON, zeros non-finite rates, requires an allowlisted plugin lifecycle state and JSON-encodable timestamps, and uses valid UTF-8 byte limits: plugin ID 256, name 256, description 4096, version 256, last error 512, JSON configuration 64 KiB, and sorted plugin lists 1024 entries. A fixed-capacity selection heap applies the list cap before copying backend snapshots, bounding Wails snapshot/event memory while comfortably exceeding ordinary local plugin inventories; overflow is deterministically omitted after sorting and produces a module validation Problem. Malformed descriptor snapshots are likewise omitted rather than truncated, so IDs are never changed. Capabilities are a fixed allowlisted set. Tracking frames, internal Application configuration, credentials/tokens, process/session identifiers, plugin executable/root paths, raw logs/errors, mutable backend objects, and stores never cross Wails. The sole permitted configuration document is the bounded owned non-credential JSON returned by `PluginsAPI.GetConfig`.
## Required tests
Diagnostic tests cover persistent JSON, rotation, memory fallback, redaction, concurrent writes/close, ingress bounds, startup error correlation and null-free plugin failure arrays across JSON serialization. These diagnostic records intentionally extend the older generic Problem boundary: messages are redacted before persistence and display. The application log destination is exposed so users can locate local files; configuration documents and unredacted internal objects remain excluded.
...
## Known gaps
The frontend and root diagnostic surface are implemented. A fresh generated project-status report remains a separate operation after source review; this task does not overwrite generated evidence.
