...
<div class="grid min-w-0 gap-1.5">
  ...
  <Select.Root bind:value={() => value, (next) => { value = next; if (next !== undefined) onValueChange?.(next) }} items={options} type="single" {disabled} {name} {required}>
    ...
    <Select.Portal>
      <Select.Content class="z-50 max-h-72 w-[var(--bits-floating-anchor-width)] min-w-[var(--bits-floating-anchor-width)] overflow-auto rounded-lg border border-border bg-surface p-1 shadow-lg shadow-black/30">
        <Select.Viewport>
