<script lang="ts">
  ...
  let filteredLogs = $derived((runtime.diagnostics?.snapshot?.entries ?? []).slice(-200).filter(entry => level === 'ALL' || entry.level === level))
  ...
  function safeSummary(): string {
    ...
    for (const [name, state] of [['Runtime', runtime.state], ['Plugins', plugins.state], ['Settings', settings.state]] as const) {
      lines.push(`${name} updated: ${localTime(state.updatedAt)}`)
      if (state.problem) lines.push(`${name} error [${safeText(state.problem.code)}]: ${safeText(state.problem.detail)}`)
    }
    ...
    if (diagnostic?.snapshot?.failure) lines.push(`Startup: ${logLine(diagnostic.snapshot.failure)}`)
    if (diagnostic?.snapshot?.logPath) lines.push(`Log file: ${diagnosticLogPath(diagnostic.snapshot.logPath)}`)
    if (diagnostic?.snapshot?.diskError) lines.push(`Disk error: ${safeText(diagnostic.snapshot.diskError)}`)
    if (diagnostic?.error) lines.push(safeText(diagnostic.error))
    for (const failure of snapshot?.pluginFailures.slice(0, 20) ?? []) lines.push(`Plugin ${safeText(failure.pluginId)} ${safeText(failure.operation)}: ${safeText(failure.message)}`)
  ...
  async function copyDiagnosticText(text: string) {
    try {
      ...
      copyStatus = '已复制'
    } catch { copyStatus = '复制失败，请重试' }
  }
</script>

<main class="page-grid min-w-0" aria-label={copy.navigation.diagnostics}>
  <PageHeader title={copy.navigation.diagnostics} description={copy.text.diagnosticsDescription} />
  {#if copyStatus}<p role="status" class="text-sm text-text-muted">{copyStatus}</p>{/if}
  ...
  <Stack gap="md">
    <section class="grid min-w-0 gap-3" aria-labelledby="module-status-title">
      ...
      <div class="surface-card grid min-w-0 divide-y divide-border">
        {#each [
          ...
          <article class="grid min-w-0 gap-1 py-2 first:pt-0 last:pb-0" aria-label={module.name}>
            ...
            {#if problemTitle(module.state.problem)}
              ...
              <p class="min-w-0 break-words text-xs text-text-muted">错误代码：{safeText(module.state.problem?.code)}</p>
              <p class="min-w-0 break-words text-sm text-text-muted">{safeText(module.state.problem?.detail)}</p>
            {/if}
          </article>
        {/each}
      </div>
    ...
    {#if runtime.diagnostics?.snapshot?.failure}
      ...
      <section class="surface-card grid gap-2 border-danger" aria-labelledby="startup-error-title">
        ...
        <p class="break-words text-sm text-text-muted">{localTime(failure.time)} · {safeText(failure.component)}/{safeText(failure.stage)} · {safeText(failure.id)}</p>
    ...
    <section class="surface-card grid gap-2" aria-labelledby="recent-logs-title">
      <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <label class="flex items-center gap-2 text-sm">日志级别
            <select class="form-control focus-ring w-auto" bind:value={level}>
              <option value="ALL">全部</option>
              {#each ['DEBUG', 'INFO', 'WARN', 'ERROR'] as value}<option {value}>{value}</option>{/each}
            </select>
          </label>
          <Button label="刷新日志" tone="secondary" disabled={runtime.diagnostics?.loading ?? false} onclick={() => void runtime.refreshDiagnostics?.()} />
          <Button label="复制日志" tone="secondary" onclick={() => void copyDiagnosticText(filteredLogs.map(logLine).join('\n').slice(0, maxDiagnosticLength))} />
        </div>
      </div>
      {#if runtime.diagnostics?.error}<p role="status" class="break-words text-sm text-warning">{safeText(runtime.diagnostics.error)}</p>{/if}
      {#if runtime.diagnostics?.snapshot?.diskError}<p class="break-words text-sm text-warning">磁盘日志不可用：{safeText(runtime.diagnostics.snapshot.diskError)}</p>{/if}
      {#if runtime.diagnostics?.snapshot?.logPath}<p class="break-words text-xs text-text-muted">日志位置：{diagnosticLogPath(runtime.diagnostics.snapshot.logPath)}</p>{/if}
      <div class="max-h-72 min-w-0 overflow-y-auto" role="region" aria-label="最近日志内容">
        {#each filteredLogs as entry}
          <article class="grid gap-1 border-b border-border py-2 last:border-b-0">
            <p class="break-words text-xs text-text-muted">{localTime(entry.time)} · <span class:text-danger={entry.level === 'ERROR'} class:text-warning={entry.level === 'WARN'}>{safeText(entry.level)}</span> · {safeText(entry.component)}/{safeText(entry.stage)} · {safeText(entry.id)}</p>
            <p class="break-words text-sm">{safeText(entry.message)}</p>
          </article>
        {:else}
          <p class="py-3 text-sm text-text-muted">{runtime.diagnostics?.loading ? '正在读取日志…' : '暂无匹配日志'}</p>
        {/each}
      </div>
    </section>
    ...
    {:else}
        ...
        {#if snapshot.osc}
          <section class="surface-card grid min-w-0 gap-3" aria-labelledby="osc-status-title">
            ...
            <DetailList items={[
              ...
              {label: copy.text.error, value: snapshot.osc.error ? safeText(snapshot.osc.error) : copy.text.none},
            ]} />
          </section>
        {:else}
      ...
      {#if snapshot.runtimeError}<p class="surface-card break-words text-sm text-warning">{safeText(snapshot.runtimeError)}</p>{/if}
